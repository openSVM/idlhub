...
fn test_output_structure() {
    ...
    assert!(output_dir.path().join("Cargo.toml").exists(), "Should generate Cargo.toml");
    assert!(output_dir.path().join("src").join("lib.rs").exists(), "Should generate src/lib.rs");
    assert!(output_dir.path().join("src").join("state.rs").exists(), "Should generate src/state.rs");
    assert!(output_dir.path().join("src").join("error.rs").exists(), "Should generate src/error.rs");
    assert!(output_dir.path().join("src").join("helpers.rs").exists(), "Should generate src/helpers.rs");
    assert!(output_dir.path().join("src").join("instructions").exists(), "Should generate instructions dir");
}
...
fn test_instruction_count() {
    ...
    assert!(instruction_files.len() >= 20,
        "Should generate at least 20 instruction files, got {}", instruction_files.len());
}
...
fn test_discriminators_are_unique() {
    ...
    assert_eq!(discriminators.len(), unique_count,
        "All discriminators should be unique. Found {} total, {} unique",
        discriminators.len(), unique_count);
}
...
fn test_state_structs_generated() {
    ...
    assert!(state_content.contains("pub struct StablePool"), "Should have StablePool struct");
    assert!(state_content.contains("pub struct FarmingPeriod"), "Should have FarmingPeriod struct");
    assert!(state_content.contains("pub struct UserFarmingPosition"), "Should have UserFarmingPosition struct");

    // Check for SIZE constants
    assert!(state_content.contains("const SIZE:"), "State structs should have SIZE constants");

    // Check for from_account_info methods
    assert!(state_content.contains("fn from_account_info"), "State structs should have from_account_info");
    assert!(state_content.contains("fn from_account_info_mut"), "State structs should have from_account_info_mut");
}
...
fn test_error_enum_generated() {
    ...
    assert!(error_content.contains("pub enum Error"), "Should have Error enum");
    assert!(error_content.contains("MathOverflow"), "Should have MathOverflow error");
    assert!(error_content.contains("SlippageExceeded"), "Should have SlippageExceeded error");
    assert!(error_content.contains("PoolPaused"), "Should have PoolPaused error");
}
...
fn test_verbose_output() {
    ...
    assert!(stdout.contains("[1/4]"), "Verbose output should show phase 1");
    assert!(stdout.contains("[2/4]"), "Verbose output should show phase 2");
